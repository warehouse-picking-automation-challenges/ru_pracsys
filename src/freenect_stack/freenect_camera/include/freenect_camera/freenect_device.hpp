...
namespace freenect_camera {
  ...
  class FreenectDevice : public boost::noncopyable {
    ...
    public:
      ...
      bool setIrBrightness(int brightness) {
        boost::lock_guard<boost::recursive_mutex> lock(m_settings_);
        uint16_t desired_brightness = brightness;
        if ((desired_brightness < 1) || (desired_brightness > 50)) {
          printf("[ERROR] Ir brightness out of range in setIrBrightness(). Requested = %d. Allowed [1-50]\n", desired_brightness);
          return (false);
        }
        else {
          int result = freenect_set_ir_brightness(device_, desired_brightness);
          if (result == 0)
            return (true);
          else
            return (false);
      ...
      int getIrBrightness() {
        boost::lock_guard<boost::recursive_mutex> lock(m_settings_);
        return (freenect_get_ir_brightness(device_));
      }

      bool setImageAutoExposure(bool value) {
        boost::lock_guard<boost::recursive_mutex> lock(m_settings_);
        int result;
        if (value)
          result = freenect_set_flag(device_, FREENECT_AUTO_EXPOSURE, FREENECT_ON);
        else
          result = freenect_set_flag(device_, FREENECT_AUTO_EXPOSURE, FREENECT_OFF);
        if (result == 0)
          return (true);
        else
          return (false);
      ...
      bool setImageAutoWhiteBalance(bool value) {
        boost::lock_guard<boost::recursive_mutex> lock(m_settings_);
        int result;
        if (value)
          result = freenect_set_flag(device_, FREENECT_AUTO_WHITE_BALANCE, FREENECT_ON);
        else
          result = freenect_set_flag(device_, FREENECT_AUTO_WHITE_BALANCE, FREENECT_OFF);
        if (result == 0)
          return (true);
        else
          return (false);
