...
#include "realsense_camera/realsense_cameraConfig.h"
...
unsigned char *rgb_registered_frame_buffer = NULL;
unsigned char *depth_frame_buffer = NULL;
uint16_t* depth16_frame_buffer = NULL;
...
std::string rgb_frame_id_, rgb_name;
std::string depth_frame_id_, ir_name;
typedef dynamic_reconfigure::Server<realsense_camera::realsense_cameraConfig> ReconfigureServer;
boost::shared_ptr<ReconfigureServer> reconfigure_server_;

int RESTART_TRIES = 0;


//depth filter callback
void depth_filter_callback(const std_msgs::Float64& msg)
{
  depth_filter = msg.data;   // read the depth filter distance in meters
}



//Definitions
sensor_msgs::CameraInfo getRgbCameraInfo(ros::Time time) 
{
  sensor_msgs::CameraInfo info;

  info = rgb_info_manager_->getCameraInfo();
...
sensor_msgs::CameraInfo getIrCameraInfo(ros::Time time)  
{
  sensor_msgs::CameraInfo info;

  info = ir_info_manager_->getCameraInfo();
...
sensor_msgs::CameraInfo getDepthCameraInfo(ros::Time time)  
{
  sensor_msgs::CameraInfo info = getIrCameraInfo(time);
...
initDepthToRGBUVMap()
{
    ...
    std::string uvmapPath = packagePath + "/data/uvmap/serial";// + useDeviceSerialNum;
...
pubRealSenseDepthImageMsg(cv::Mat& depth_mat)
{
  ...
  sensor_msgs::Image depth_img_reg;
	...
	depth_img.encoding = sensor_msgs::image_encodings::TYPE_16UC1;//MONO8;
	depth_img.is_bigendian = 0;

	int step = sizeof(unsigned char) * depth_img.width;
  int step16 = sizeof(uint16_t) * depth_img.width;
	int size = step16 * depth_img.height;//step * depth_img.height;
	depth_img.step = step16;//step;
	depth_img.data.resize(size);
	memcpy(&depth_img.data[0], depth_mat.data, size);
  depth_img_reg = depth_img;
  depth_img_reg.header.frame_id = "/"+rgb_frame_id;
  depth_img.header.frame_id = "/"+depth_frame_id;
	// realsense_depth_image_pub.publish(depth_img);
  pub_depth_.publish(depth_img,getDepthCameraInfo(ros::Time::now()));
  pub_depth_registered_.publish(depth_img_reg,getDepthCameraInfo(ros::Time::now()));
...
pubRealSenseInfraredImageMsg(cv::Mat& ir_mat)
{
    ...
    pub_ir_.publish(ir_img,getDepthCameraInfo(ros::Time::now()));
}
#endif
...
pubRealSenseRGBImageMsg(cv::Mat& rgb_mat)
{
  ...
  cv::Mat rgb_scaled_mat;//(480, 640, CV_8UC3);
  cv::resize(rgb_mat, rgb_scaled_mat, cv::Size(640,480));
	rgb_img.header.stamp = ros::Time::now();

	rgb_img.width = rgb_scaled_mat.cols; //rgb_mat.cols;
	rgb_img.height = rgb_scaled_mat.rows; //rgb_mat.rows;
	...
	memcpy(&rgb_img.data[0], rgb_scaled_mat.data, size);//memcpy(&rgb_img.data[0], rgb_mat.data, size);
  rgb_img.header.frame_id = "/"+rgb_frame_id;
	// realsense_rgb_image_pub.publish(rgb_img);
  // pub_rgb_.publish(rgb_img, getRgbCameraInfo(ros::Time::now()));
  pub_rgb_.publish(rgb_img, getDepthCameraInfo(rgb_img.header.stamp));
...
void processDepth()
{
    ...
    if(depth_stream.fd == -42)
    {
      cout<< "\nfd value depth "<<(depth_stream.fd);
      restart();
    }
    else
    {
...
bool validDepth(float depth_)
{
    return (depth_>0.000001f && (depth_*depth_scale > depth_filter));
}
...
processRGBD()
{
  ...
  Mat depth16_frame(depth_stream.height, depth_stream.width, CV_16UC1, depth16_frame_buffer);
  ...
  Mat rgb_registered_frame(480,640, CV_8UC3, rgb_registered_frame_buffer);
	//YUV 2 RGB
...
//            depth_max = new_max;
//        }
        //Depth scaled over 0 to 255
        depth_frame_buffer[i] = depth ? 255 * (sensor_depth_max - depth) / sensor_depth_max : 0;
        //Depth in millimeters
        if(validDepth(depth))
        {
          depth16_frame_buffer[i] = depth;
        }
        else
        {
          //TODO Appropriate bogus depth value
          depth16_frame_buffer[i] = 0;

          rgb_registered_frame_buffer[i*3] = 0;
          rgb_registered_frame_buffer[i*3+1] = 0;
          rgb_registered_frame_buffer[i*3+2] = 0;
        }
        ...
        if(validDepth(depth))
        {
            ...
            if(isHaveD2RGBUVMap)
            {
                    ...
                    rgb_registered_frame_buffer[i*3] = b;
                    rgb_registered_frame_buffer[i*3+1] = g;
                    rgb_registered_frame_buffer[i*3+2] = r;
    ...
    pubRealSenseDepthImageMsg(depth16_frame);
    // pubRealSenseRGBImageMsg(rgb_frame);
    pubRealSenseRGBImageMsg(rgb_registered_frame);
...
// sensor_msgs::CameraInfoPtr getRgbCameraInfo(ros::Time time) 
// {
...
// sensor_msgs::CameraInfoPtr getIrCameraInfo(ros::Time time)  
// {
//   sensor_msgs::CameraInfoPtr info;
...
// sensor_msgs::CameraInfoPtr getDepthCameraInfo(ros::Time time)  
// {
...
//   return info;
// }
void configCb(realsense_camera::realsense_cameraConfig &config, uint32_t level)
{
...
int initStreams(VIDEO_DEVICE& matched_realsense, std::string serial_number="")
{
  //find realsense video device
    ...
    if(1)
    {
      ...
      {
        if(serial_number=="" || video_lists[i].serial_number == serial_number)
        {
          matched_realsense = video_lists[i];
          found_device_serial = 1;
        }
    ...
    if(found_device_serial == 0)
    {
      printf("Intel(R) RealSense(TM) 3D Camer video device serial number mismatch error!!!!!!!!!!!\n");
      // ros::shutdown();
      return 0;
    }
    //return 0;

    if(matched_realsense.video_names.size() < 2)
    {
    ...
    {
      useDeviceSerialNum = matched_realsense.serial_number;
...
void attachStreamToDevice(VIDEO_DEVICE realsense_device)
{

    strncpy(rgb_stream.videoName, realsense_device.video_names[0].c_str(), realsense_device.video_names[0].length());
    strncpy(depth_stream.videoName, realsense_device.video_names[1].c_str(), realsense_device.video_names[1].length());
    ...
    printf("RealSense Camera is running again!\n");

}

void restart()
{
  VIDEO_DEVICE device_;
  // capturer_mmap_exit(&rgb_stream);
  // capturer_mmap_exit(&depth_stream);
  // initVideoStream();
  if(initStreams(device_,camera_serial))
  {
    attachStreamToDevice(device_);
  }
  else
  {
    cout<<"Failed to restart";
  }  
  // cout<<"\n\n\n\n~~~~~~~~~~~~~~~~~~~~~~~~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^RESTARTING " << RESTART_TRIES << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&$$$$$$$$$$$\n\n\n\n\n\n";
  RESTART_TRIES ++;
}
...
int main(int argc, char* argv[])
{
    ...
    double depth_filter_d;
    private_node_handle_.param("depth_filter", depth_filter_d, 0.30);
    depth_filter = depth_filter_d;
    ...
    private_node_handle_.param("camera", camera_name, std::string("camera_blah"));
    private_node_handle_.param("camera_serial", camera_serial, std::string(""));
    if(camera_serial!="")
    {
      camera_serial = camera_serial.substr(1);
    }
    ...
    std::string target_device_name = "Intel(R) RealSense(TM) 3D Camer";
    std::vector<VIDEO_DEVICE> video_lists;
    VIDEO_DEVICE matched_realsense;
    bool found_device_serial = false;
    list_devices(target_device_name, video_lists);
    ...
    if(1)
    {
    	...
    	for(int i=0; i<video_lists.size(); ++i)
    	{
        if(camera_serial=="" || video_lists[i].serial_number == camera_serial)
        {
          matched_realsense = video_lists[i];
          found_device_serial = true;
        }
    		printf("\nPCI: %s\n", video_lists[i].card_name.c_str());
    		printf("Serial: %s\n", video_lists[i].serial_number.c_str());
    		for(int j=0; j<video_lists[i].video_names.size(); ++j)
   ...
   //  	printf("use camera %s\n", useDeviceSerialNum.c_str());
   //  }
    if(!found_device_serial)
    {
      printf("Intel(R) RealSense(TM) 3D Camer video device serial mismatch error!!!!!!!!!!!\n");
      ros::shutdown();
      return 0;
    }
    if(matched_realsense.video_names.size() < 2)
    ...
    strncpy(rgb_stream.videoName, matched_realsense.video_names[0].c_str(), matched_realsense.video_names[0].length());
    strncpy(depth_stream.videoName, matched_realsense.video_names[1].c_str(), matched_realsense.video_names[1].length());
    ...
    rgb_registered_frame_buffer = new unsigned char[480*640*3];
    depth_frame_buffer = new unsigned char[depth_stream.width * depth_stream.height];
    depth16_frame_buffer = new uint16_t[depth_stream.width * depth_stream.height];
    ...
    //Depth filter
    ros::Subscriber depth_filter_sub = n.subscribe(camera_name+"/depth_filter", 1, depth_filter_callback);



    //Set up realsense ROS camera pubsub
    image_transport::ImageTransport it(n);
    ros::NodeHandle rgb_nh(n, camera_name+"/rgb");
    image_transport::ImageTransport rgb_it(rgb_nh);
    ros::NodeHandle ir_nh(n, camera_name+"/ir");
    image_transport::ImageTransport ir_it(ir_nh);
    ros::NodeHandle depth_nh(n, camera_name+"/depth");
    image_transport::ImageTransport depth_it(depth_nh);
    ros::NodeHandle depth_registered_nh(n, camera_name+"/depth_registered");
    image_transport::ImageTransport depth_registered_it(depth_registered_nh);
    private_node_handle_.param("rgb_frame_id",   rgb_frame_id_,   string("/openni_rgb_optical_frame"));
    private_node_handle_.param("depth_frame_id", depth_frame_id_, string("/openni_depth_optical_frame"));
    rgb_name = "rgb";
    ir_name  = "depth";
    std::string rgb_info_url, ir_info_url;
    private_node_handle_.param("rgb_camera_info_url", rgb_info_url, string("package://realsense_camera/calibration/rgb/cal.yml"));
    private_node_handle_.param("depth_camera_info_url", ir_info_url, string("package://realsense_camera/calibration/depth/cal.yml"));
    ...
    pub_ir_ = ir_it.advertiseCamera("image_raw", 1);
    pub_depth_ = depth_it.advertiseCamera("image_raw", 1);
    pub_depth_registered_ = depth_registered_it.advertiseCamera("image_raw", 1);
    ros::NodeHandle recon_nh(n, camera_name+"/driver");
    reconfigure_server_.reset( new ReconfigureServer(recon_nh) );
    reconfigure_server_->setCallback(configCb);



    ros::Rate loop_rate(30);

    while(ros::ok() && RESTART_TRIES<10)
    {
        processRGBD();
    ...
    delete[] rgb_registered_frame_buffer;
