...
##############################################################################################
# Reflex Gripper Class                                                                      #
##############################################################################################

from reflex_gripper import *
...
class TaskPlanner:
    ...
    def __init__(self):
        ...
        self.ik_computed_configuration = None

        self.tf_broadcaster = tf.TransformBroadcaster()

        self.first_video = False
        self.execution_time = -1
        self.grasp_eval_time = -1
        ...
        if rospy.has_param('/data_gathering'):
            self.data_gathering = rospy.get_param("/data_gathering")
        else:
            self.data_gathering = False
        print "data_gathering: " + str(self.data_gathering)
        ...
        if self.move_to_state( LEFT_ARM, self.states[LEFT_ARM]['origin_state'], ROBOT_SPEED_FAST,"origin") == False:
            sys.exit(-1)
        # self.right_gripper = RobotiqGripper()
        self.right_gripper = ReFlexHand( SIMULATING )
        ...
        for arm in 0, 1: # 0 = left, 1 = right
            ...
            for bin in 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L':
                ...
                if arm == 1 and bin in ['A', 'B', 'C', 'D',  'F', 'G', 'H', 'I']:
                    ...
                    another_helpful_orientation = Quaternion( 0.0, 0.0, 0.707107, 0.707107 )
                    pose_i.orientation = rotate_quaternion( pose_i.orientation, another_helpful_orientation )
                #LEFT ARM
                ...
                reflex_heights = {'A': 0.04, 'B': 0.04, 'C': 0.04, 'D': 0.04, 'E': 0.04, 'F': 0.04, 'G': 0.04, 'H': 0.04, 'I': 0.04, 'J': 0.04, 'K': 0.04, 'L': 0.04}
                map_heights = [ unigripper_heights, reflex_heights ]
    ...
    def task_planner(self,timer_val):
        ...
        if self.g_decision_making_state_available == True:
            ...
            if current_motion_planning_mode == 6: # mp_to_O -> move to origin
                ...
                if self.first_video and self.video_p != False:
                    self.video_p.terminate()
                self.video_p = False
                next_pose = input('Paused: Give next pose when ready to resume: ' )
                self.current_pose = int( next_pose )

                f = open("icra_poses/"+self.g_decision_making_state.object_name+".yaml");

                pose_count=1
                for line in f:
                    if pose_count == self.current_pose:
                        nums = line.split();
                        HARD_CODED_OBJECT_POSITION.position.x = float(nums[0])
                        HARD_CODED_OBJECT_POSITION.position.y = float(nums[1])
                        HARD_CODED_OBJECT_POSITION.position.z = float(nums[2])
                        HARD_CODED_OBJECT_POSITION.orientation.x = float(nums[3])
                        HARD_CODED_OBJECT_POSITION.orientation.y = float(nums[4])
                        HARD_CODED_OBJECT_POSITION.orientation.z = float(nums[5])
                        HARD_CODED_OBJECT_POSITION.orientation.w = float(nums[6])
                        break
                    else:
                        pose_count+=1
                f.close()
                self.grasp_reason = "Reason"
...
#                         move_ok = False
#                         while move_ok == False:
#                             print "Moving to configuration failed. Going to try again..."
#                             move_ok = self.move_to_config( CAMERA, self.g_decision_making_state.bin_id, 'detect_config_1', ROBOT_SPEED_FAST )
...
#                         while move_ok == False:
#                             print "Moving to configuration failed. Going to try again..."
#                             move_ok = self.move_to_config( CAMERA, self.g_decision_making_state.bin_id, 'detect_config_2', ROBOT_SPEED_FAST )
...
#                         self.state = 10 # MOVED_TO_DETECT1 = 10
            ...
            if current_motion_planning_mode == 16: # mp_to_D2 -> Move to Detect 2
                ...
                # if FAST_DETECTION_MODE:
                self.state = 11 # MOVE TO DETECT 2
    #             else:
    #                 rospy.sleep(ROBUST_POSE_TIMEOUT)
    ...
    #                     while move_ok == False:
    #                         print "Moving to configuration failed. Going to try again..."
    #                         move_ok = self.move_to_config( CAMERA, self.g_decision_making_state.bin_id, 'map_config', ROBOT_SPEED_FAST )
    ...
    #                     self.state = 11 # MOVED_TO_DETECT2 = 11
            ...
            if current_motion_planning_mode == 9: # mp_to_Ob_ -> move closer to object
                ...
                self.map_and_detect()
                self.grasp_eval_start = time.time()

            if current_motion_planning_mode == 17: # mp_to_eval -> evaluate grasp
                print "============== Evaluate Grasp =============="
                self.evaluate_grasp()
                grasp_end_time = time.time()
                self.grasp_eval_time = grasp_end_time - self.grasp_eval_start
                self.execution_start_time = grasp_end_time
                ...
                if self.moving_arm == 0:
                    ...
                    arm = "right"
                self.video_start_time = str( int( round( time.time() ) ) )
                if RECORDING_VIDEO:
                  args = ['avconv', '-f', 'video4linux2', '-s', '640x480', '-r', '25', '-qscale', '1', '-t', '5', '-i', '/dev/video1', '-vf','transpose=1,transpose=1','/media/DATA/icra_videos/' + self.g_decision_making_state.object_name + "_" + arm + "_" + str( self.current_pose ) + "_" + self.video_start_time + '.mp4']
                  self.video_p = False
                  self.video_p = subprocess.Popen(args)
                  self.first_video = True
                  rospy.sleep(1)

                # print "Grasp Eval Time: " + str( self.grasp_eval_time )
            ...
            if current_motion_planning_mode == 10: # mp_to_AB_ -> move away from bin
                ...
                self.execution_time = execution_end_time - self.execution_start_time
            ...
            if current_motion_planning_mode == 11: # mp_to_OB -> move to order bin
                ...
                if self.move_to_state( self.moving_arm, self.states[self.moving_arm]['order_state'], ROBOT_SPEED_FAST, 'order_bin' ) == False:
                    ...
                    while done_trying == False:
                        ...
                        if counter >= 6:
                            ...
                            if self.move_to_config( END_EFFECTOR, self.g_decision_making_state.bin_id, 'backup_config', ROBOT_SPEED_FAST ):
                                ...
                                if self.move_to_general_config( END_EFFECTOR, temp_pose, ROBOT_SPEED_FAST ) == False:
                    ...
                    if counter >= 6:
                        self.state = 13
                        self.grasp_computed = False
                        self.grasp_reason = "Unable to retract with object."
                    else:
                        self.state = 6
                ...
                if self.moving_arm == RIGHT_ARM:
                    ...
                    self.right_gripper.setGraspMode( self.right_gripper.GM_basic )
                else:
    ...
    def output_statistics( self ):
        ...
        f.write( str(self.video_start_time) + ", " + self.g_decision_making_state.object_name + ", " + arm + ", " + str( self.current_pose ) + ", " + str( self.grasp_eval_time ) + ", " + str( self.execution_time ) + ", " + str( self.total_time ) + ", "+str(self.grasps_tried)+", "+str(int(self.grasp_computed))+", "+self.grasp_reason + "\n" )
        f.close()
    ...
    def move_to_general_config(self,link,config,speed):
        ...
        try:
            ...
            sys.exit(-1)
        if (resp.success):
            self.ik_computed_configuration = dict(zip(resp.arm_config.name, resp.arm_config.position))
            # self.compute_plan(ikreq.arm, self.ik_computed_configuration, speed)
    ...
    def get_object_pose( self ):
        ...
        # self.object_pose.position.x += 0.2
        # self.object_pose.position.x += random.uniform( +0.15, +0.25 )#.13
        # self.object_pose.position.y += random.uniform( -0.05, +0.05 )#0
        # self.object_pose.position.z += random.uniform( -0.1, -0.04 )#-.04
    ...
    def evaluate_grasp( self ):
        ...
        if not self.compute_grasp_call():
            self.remove_object_from_scene(self.g_decision_making_state.object_name)
            self.state = 8
            #We should also report a failure and a reason here?
            self.grasp_computed = False
            self.grasp_reason = "Failed to compute a valid grasp"
            total_end_time = time.time()
            self.video_start_time = total_end_time
            self.grasp_eval_time = -1
            self.execution_time = -1
            self.total_time = total_end_time - self.global_start_time
    ...
    def compute_grasp_call(self):
        ...
        if success_grasp == True:
            if resp.success:
                self.grasp_computed = True
                self.grasp_id = resp.grasp_id
                self.grasps_tried = resp.num_grasps_tried
                if resp.arm == "left":
    ...
    def remove_all_objects(self):
        ...
        rospy.wait_for_service('clear_objects')
        req = clear_objectsRequest();
        try:
            clear_object_caller = rospy.ServiceProxy('clear_objects', clear_objects)
    ...
    def grasp(self,bin_id):
        ...
        if self.moving_arm == RIGHT_ARM:
            if self.grasp_id == 1: # NORMAL GRASP
                self.right_gripper.open("bin_open")
                self.right_gripper.setGraspMode( self.right_gripper.GM_basic )
            elif self.grasp_id == 2: #PINCH GRASP
                self.right_gripper.open("bin_open")
                self.right_gripper.setGraspMode( self.right_gripper.GM_pinch )
                self.right_gripper.reflex_hand.move_finger(2, self.right_gripper.maxJointAngle)
            elif self.grasp_id == 3: #FINGERTIPS
                self.right_gripper.open("bin_open")
                self.right_gripper.setGraspMode( self.right_gripper.GM_basic )
            else:
                print "Some grasp ID that I do not understand is here!"
                sys.exit(0)
