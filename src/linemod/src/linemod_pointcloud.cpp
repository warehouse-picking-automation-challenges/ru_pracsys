...
/** fill the point cloud with the 3D points */
void LinemodPointcloud::fill(const std::vector<cv::Vec3f> & pts, cv::Mat color)
{
  int size_old = modifier->size();
  modifier->resize(size_old + pts.size());
  sensor_msgs::PointCloud2Iterator<float> iter_x(pc_msg, "x");
  ...
  for(; it_data != pts.end(); ++it_data, ++iter_x, ++iter_y, ++iter_z, ++iter_r, ++iter_g, ++iter_b)
  {
    ...
    *iter_y = (*it_data)(1);
    *iter_z = (*it_data)(2);
    cv::Vec3b intensity = color.at<cv::Vec3b>(y, x);
    *iter_r = intensity.val[2];
    *iter_g = intensity.val[1];
    *iter_b = intensity.val[0];
    //printf("x = %d, y = %d\n", x, y);
    x++;
    if (x == color.cols)
    {
