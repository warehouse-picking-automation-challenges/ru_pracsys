...
#define X_KINECT_DISTANCE_FROM_JOINT  0.0 /* in meters */
#define Y_KINECT_DISTANCE_FROM_JOINT  0.045   /* in meters */
#define Z_KINECT_DISTANCE_FROM_JOINT  0.125 /* in meters */
...
int baxter_cam1_first_rotating_axis;
double baxter_cam1_first_rotating_axis_signal;
int baxter_cam1_second_rotating_axis;
double baxter_cam1_second_rotating_axis_signal;

int baxter_cam2_first_rotating_axis;
double baxter_cam2_first_rotating_axis_signal;
int baxter_cam2_second_rotating_axis;
double baxter_cam2_second_rotating_axis_signal;

int motoman_cam1_first_rotating_axis;
double motoman_cam1_first_rotating_axis_signal;
int motoman_cam1_second_rotating_axis;
double motoman_cam1_second_rotating_axis_signal;

int motoman_cam2_first_rotating_axis;
double motoman_cam2_first_rotating_axis_signal;
int motoman_cam2_second_rotating_axis;
double motoman_cam2_second_rotating_axis_signal;
...
tf::StampedTransform
compute_kinect_pose_transform_robot_arm(int first_rotating_axis, double first_rotating_axis_signal,
        int second_rotating_axis, double second_rotating_axis_signal)
{
    ...
    if (first_rotating_axis == 1)
        transform.rotate(Eigen::AngleAxisd(first_rotating_axis_signal * theta, Eigen::Vector3d::UnitX()));
    else if (first_rotating_axis == 2)
        transform.rotate(Eigen::AngleAxisd(first_rotating_axis_signal * theta, Eigen::Vector3d::UnitY()));
    else if (first_rotating_axis == 3)
        transform.rotate(Eigen::AngleAxisd(first_rotating_axis_signal * theta, Eigen::Vector3d::UnitZ()));

    if (second_rotating_axis == 1)
        transform.rotate(Eigen::AngleAxisd(second_rotating_axis_signal * theta, Eigen::Vector3d::UnitX()));
    else if (second_rotating_axis == 2)
        transform.rotate(Eigen::AngleAxisd(second_rotating_axis_signal * theta, Eigen::Vector3d::UnitY()));
    else if (second_rotating_axis == 3)
        transform.rotate(Eigen::AngleAxisd(second_rotating_axis_signal * theta, Eigen::Vector3d::UnitZ()));

    tf::transformEigenToTF(transform, tf_transform);
    tf_transform.setOrigin(tf::Vector3(X_KINECT_DISTANCE_FROM_JOINT, Y_KINECT_DISTANCE_FROM_JOINT, Z_KINECT_DISTANCE_FROM_JOINT));
...
compute_kinect_pose_transform()
{
    if (g_robot == BAXTER)
    {
        if (g_robot_arm == LEFT_ARM)
            return compute_kinect_pose_transform_robot_arm(
                    baxter_cam1_first_rotating_axis,
                    baxter_cam1_first_rotating_axis_signal,
                    baxter_cam1_second_rotating_axis,
                    baxter_cam1_second_rotating_axis_signal);
        else
            return compute_kinect_pose_transform_robot_arm(
                    baxter_cam2_first_rotating_axis,
                    baxter_cam2_first_rotating_axis_signal,
                    baxter_cam2_second_rotating_axis,
                    baxter_cam2_second_rotating_axis_signal);
    }
    else
    {
        if (g_robot_arm == LEFT_ARM)
            return compute_kinect_pose_transform_robot_arm(
                    motoman_cam1_first_rotating_axis,
                    motoman_cam1_first_rotating_axis_signal,
                    motoman_cam1_second_rotating_axis,
                    motoman_cam1_second_rotating_axis_signal);
        else
            return compute_kinect_pose_transform_robot_arm(
                    motoman_cam2_first_rotating_axis,
                    motoman_cam2_first_rotating_axis_signal,
                    motoman_cam2_second_rotating_axis,
                    motoman_cam2_second_rotating_axis_signal);
    }
}
...
publish_localize_transform(const sensor_msgs::ImageConstPtr& depth, std::string camera_link_tf_topic)
{
	...
	if (g_listener->canTransform(camera_link_tf_topic, "/mapping_camera_frame", ros::Time(0)))
	{
		...
		got_transform1 = true;
		ROS_INFO_STREAM("got transform 1");
	}
	...
	if (g_listener->canTransform("/kinect_pose_in_the_arm", g_base_reference_frame, ros::Time(0)))
	{
		...
		got_transform2 = true;
		ROS_INFO_STREAM("got transform 2");
	}
	...
	if (got_transform1 && got_transform2)
	{
		if (!got_both_transform)
		{
			...
			ROS_INFO_STREAM("got both transforms");
		}
	...
	if (got_all_required_transforms)
	{
	    ...
	    {
            ROS_INFO_STREAM("HAVE " << camera_link_tf_topic.c_str() << " -> /mapping_camera_frame transform");
            save_localize_transform();
	    }
	    else
	    {
	        g_localize_transform_timestamp = depth->header.stamp;
	        ROS_INFO_STREAM("DO NOT HAVE1 " << camera_link_tf_topic.c_str() << " -> /mapping_camera_frame transform");
	    }
	}
	else
	{
        g_localize_transform_timestamp = depth->header.stamp;
		ROS_INFO_STREAM("DO NOT HAVE2 " << camera_link_tf_topic.c_str() << " -> /mapping_camera_frame transform" );
	}

    ROS_INFO_STREAM("WORLD tf ts " << g_localize_transform_timestamp);
...
publish_kinect_pose_in_the_arm_transform(std::string arm_joint_tf_topic)
{
	...
	if (g_listener->canTransform(g_base_reference_frame, arm_joint_tf_topic, ros::Time(0)))
	{
        ...
        ROS_INFO_STREAM("ATTENTION: " << arm_joint_tf_topic.c_str() << " -> kinect_pose_in_the_arm transform missing");
}
...
{
    ROS_INFO_STREAM("======== localizer dynamic_reconfiguration_handler() called." );

    baxter_cam1_first_rotating_axis = c.baxter_cam1_first_rotating_axis;
    baxter_cam1_first_rotating_axis_signal = c.baxter_cam1_first_rotating_axis_signal;
    baxter_cam1_second_rotating_axis = c.baxter_cam1_second_rotating_axis;
    baxter_cam1_second_rotating_axis_signal = c.baxter_cam1_second_rotating_axis_signal;

    baxter_cam2_first_rotating_axis = c.baxter_cam2_first_rotating_axis;
    baxter_cam2_first_rotating_axis_signal = c.baxter_cam2_first_rotating_axis_signal;
    baxter_cam2_second_rotating_axis = c.baxter_cam2_second_rotating_axis;
    baxter_cam2_second_rotating_axis_signal = c.baxter_cam2_second_rotating_axis_signal;

    motoman_cam1_first_rotating_axis = c.motoman_cam1_first_rotating_axis;
    motoman_cam1_first_rotating_axis_signal = c.motoman_cam1_first_rotating_axis_signal;
    motoman_cam1_second_rotating_axis = c.motoman_cam1_second_rotating_axis;
    motoman_cam1_second_rotating_axis_signal = c.motoman_cam1_second_rotating_axis_signal;

    motoman_cam2_first_rotating_axis = c.motoman_cam2_first_rotating_axis;
    motoman_cam2_first_rotating_axis_signal = c.motoman_cam2_first_rotating_axis_signal;
    motoman_cam2_second_rotating_axis = c.motoman_cam2_second_rotating_axis;
    motoman_cam2_second_rotating_axis_signal = c.motoman_cam2_second_rotating_axis_signal;
}
...
get_robot_name()
{
    ...
    if (ros::param::get("/robot_name", robot_name))
    {
        ...
        {
            ROS_FATAL_STREAM("Error: unkown robot named " << robot_name);
            exit(1);
        }
        ROS_INFO_STREAM("robot name: " << robot_name.c_str() );
    }
    else
    {
        ROS_INFO_STREAM("No robot name specified. So, we will use baxter configuration." );
        g_robot = BAXTER;
    }
}
