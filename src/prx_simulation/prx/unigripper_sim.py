    ...
    for outer_loop in xrange(1,len(trajectory.points)):
      ...
      for x in xrange(len(trajectory.joint_names)):
        ...
        if math.fabs(target_point.positions[x] - current_point.positions[x])/(diff.secs+diff.nsecs/1000000000.0) > vel:
          ...
          rospy.logerr('Unigripper sim : %s requested velocity %f (given by the time_from_start value) is too large (max %f). Aborting...',trajectory.joint_names[x],(target_point.positions[x] - current_point.positions[x])/(diff.secs+diff.nsecs/1000000000.0),vel)
          success = False
          break
