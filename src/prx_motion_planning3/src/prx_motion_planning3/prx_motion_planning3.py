...
X_CLOSE_TO_OBJECT = 0.075
X_CLOSE_TO_OBJECT2 = 0.0
...
def get_object_pose():
    ...
    if HARD_CODED_OBJECT == True:
        ...
        if g_robot == BAXTER:  
            ...
            object_pose.position.x = 1.03 #trans[0]
            object_pose.position.y = 0.28 #trans[1]
            object_pose.position.z = 0.93 #trans[2]
            object_pose.orientation.x = -0.477072525368 #rot[0]
            object_pose.orientation.y = 0.534384847975 #rot[1]
            object_pose.orientation.z = -0.488505960788 #rot[2]
            object_pose.orientation.w = 0.498193301911 #rot[3]
...
def robotiq_state_callback(robot_input):
    global g_robotiq_state
    g_robotiq_state = robot_input 


class FutureMotomanGripper():
    def __init__(self):
        global g_robotiq_state
        g_robotiq_state = SModel_robot_input();
        g_robotiq_state.gSTA = 3
        self.GM_basic = 0
        self.GM_pinch = 1
        self.GM_scissor = 2
        self.GM_wide = 3
        ...
        self.publisher = rospy.Publisher('/SModelRobotOutput',
                                         SModel_robot_output)
        self.subscriber = rospy.Subscriber('/SModelRobotInput',SModel_robot_input,robotiq_state_callback)
        ...
        rospy.sleep(1);
        while True:
            state = g_robotiq_state
            if state.gSTA!=0:
                break
        self.fake_flag = False

    # def __init__(self,fake_flag):
    #     self.fake_flag = True;
...
def set_and_calibrate_gripper(gripper_name):
    ...
    if g_robot == BAXTER:
        ...
        if gripper_name == "left":
            g_gripper = UnigripperGripper()
            g_gripper.reset()

    return


def set_arm_and_move_group(robot_arm):
    global g_robot
    ...
    if robot_arm == 0:
        ...
        if g_robot == BAXTER:
            g_move_group_name = 'right_arm'
        else:
            g_move_group_name = 'arm_right'
    set_move_group(g_move_group_name)
...
def attach_object_to_the_gripper():
    ...
    if g_object_attached_to_the_gripper == False:
        if g_robot == BAXTER:
            ...
            if g_decision_making_state.robot_arm == 0:
                gripper_name = 'head_sponge'
            else:
                gripper_name = 'arm_right_link_7_t'
            attaching_pose_in_gripper = PoseStamped()
            attaching_pose_in_gripper.header = Header(stamp = rospy.Time.now(), frame_id = gripper_name)
            attaching_pose_in_gripper.pose.position = Point(*[0.0, 0.0, 0.04]) # TODO: This constant is object dependent
...
def dettach_object_from_the_gripper():
    ...
    if g_object_attached_to_the_gripper == True:
        if g_robot == BAXTER:
            ...
            if g_decision_making_state.robot_arm == 0:
                remove_mesh_from_robot("head_sponge", g_decision_making_state.object_name + '_attached')
            else:
                remove_mesh_from_robot("head_sponge", g_decision_making_state.object_name + '_attached')
...
def grasp():
    ...
    g_gripper.close() # initiate grasp!
    move_ok = plan_and_move_end_effector_to(g_decision_making_state.robot_arm, 
                                            object_grasp_pose, 
                                            ROBOT_SPEED_SLOW, 
                                            g_bin_positions[g_robot][g_decision_making_state.robot_arm][g_decision_making_state.bin_id]['map_robot_position_2_ik_seed'],
    ...
    if move_ok == True:
        ...
        if move_ok == True:
            ...
            if g_robot==MOTOMAN and g_decision_making_state.robot_arm==1:
                g_gripper.open("bin_open")
            else:
                g_gripper.open();
...
def motion_planner():
    ...
    if g_decision_making_state_available == True:
        ...
        if current_motion_planning_mode == 7: # mp_to_B -> move to Bin
            ...
            #    g_bin_positions[g_robot][g_decision_making_state.robot_arm][g_decision_making_state.bin_id]['map_robot_position_2_ik_seed'])
            set_bin_center_pose(g_bin_positions[g_robot][g_decision_making_state.robot_arm][g_decision_making_state.bin_id]['bin_center'])
            move_ok = False
            while move_ok == False:
                move_ok = plan_and_move_end_effector_to(g_decision_making_state.robot_arm, 
                                                        g_bin_positions[g_robot][g_decision_making_state.robot_arm][g_decision_making_state.bin_id]['map_robot_position_2'], 
                                                        ROBOT_SPEED_FAST, 
                                                        g_bin_positions[g_robot][g_decision_making_state.robot_arm][g_decision_making_state.bin_id]['map_robot_position_2_ik_seed'],
                                                        MAP_CONSTRAINTS)
            add_shelf_to_scene()    
...
def decision_making_state_handler(data):
    ...
    if g_decision_making_state.robot_arm != g_previous_arm:
        ...
        print "DM change: "+str(g_decision_making_state.robot_arm)
        set_arm_and_move_group(g_decision_making_state.robot_arm)
...
def motoman_joints_state_handler(data):
    ...
    for key in data.name:
        if "palm" in key or "finger" in key or "head_hinge" in key:
            #nothing
            var = 1
        else:
...
def compute_scene_poses():
    ...
    for robot in 0, 1: # 0 = BAXTER, 1 = MOTOMAN
        ...
        if robot == BAXTER:
            ...
            x_origin = g_shelf_description.x - 0.09
            x_mapping = g_shelf_description.x - 0.09
            for arm in 0, 1: # 0 = left, 1 = right
                g_bin_positions[robot][arm] = {}
                for bin in 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L':
                    g_bin_positions[robot][arm][bin] = {}
                    ...
                    # g_bin_positions[robot][arm][bin]['origin_robot_position'] = pose
                    if arm == 0: # left
                        g_bin_positions[robot][arm][bin]['origin_robot_configuration'] = {'torso_joint_b1': 0, 'arm_left_joint_1_s': 1.57,'arm_left_joint_2_l': 0,'arm_left_joint_3_e': 0,'arm_left_joint_4_u': -1.70,'arm_left_joint_5_r': 0,'arm_left_joint_6_b': 0,'arm_left_joint_7_t': 0,'head_hinge':0}
                        # g_bin_positions[robot][arm][bin]['origin_robot_configuration'] = {'torso_joint_b1': 0, 'arm_left_joint_1_s': 0.0,'arm_left_joint_2_l': 0,'arm_left_joint_3_e': 0,'arm_left_joint_4_u': 0.0,'arm_left_joint_5_r': 0,'arm_left_joint_6_b': 0,'arm_left_joint_7_t': 0,'head_hinge':0}
                        g_bin_positions[robot][arm][bin]['order_bin_robot_configuration'] = g_bin_positions[robot][arm][bin]['origin_robot_configuration']#{'torso_joint_b1': 0, 'arm_left_joint_1_s': 1.396,'arm_left_joint_2_l': 1.57,'arm_left_joint_3_e': 0,'arm_left_joint_4_u': -0.2,'arm_left_joint_5_r': 0,'arm_left_joint_6_b': 0,'arm_left_joint_7_t': 0}
                    else:
                        g_bin_positions[robot][arm][bin]['origin_robot_configuration'] = {'torso_joint_b1': 0, 'arm_right_joint_1_s': 1.57,'arm_right_joint_2_l': 0,'arm_right_joint_3_e': 0,'arm_right_joint_4_u': -1.7,'arm_right_joint_5_r': 0,'arm_right_joint_6_b': 0,'arm_right_joint_7_t': 0}
                        g_bin_positions[robot][arm][bin]['order_bin_robot_configuration'] = g_bin_positions[robot][arm][bin]['origin_robot_configuration']#{'torso_joint_b1': 0, 'arm_right_joint_1_s': 1.396,'arm_right_joint_2_l': 1.57,'arm_right_joint_3_e': 0,'arm_right_joint_4_u': -0.2,'arm_right_joint_5_r': 0,'arm_right_joint_6_b': 0,'arm_right_joint_7_t': 0}
                    
                    y_center_bin, bin_width = get_y_coordinate_of_the_center_of_the_bin(bin)
                    z_center_bin, bin_height = get_z_coordinate_of_the_center_of_the_bin(bin)
            
                    pose_i = copy.deepcopy(pose)
                    pose_i.position.x = x_origin
                    pose_i.position.y = y_center_bin
                    pose_i.position.z = z_center_bin
                    g_bin_positions[robot][arm][bin]['bin_robot_position'] = pose_i
                    ...
                    pose_k.position.z -= 0.12
                    pose_k.position.x = x_mapping
                    g_bin_positions[robot][arm][bin]['map_robot_position_2'] = pose_k
                    g_bin_positions[robot][arm][bin]['map_robot_position_2_ik_seed'] = g_bin_positions[robot][arm][bin]['origin_robot_configuration']
            
                    pose_bin_center = copy.deepcopy(pose_i)
                    pose_bin_center.position.x = g_shelf_description.x
                    g_bin_positions[robot][arm][bin]['bin_center'] = pose_bin_center
...
def motion_planner_init():
    ...
    g_joints_state = {'torso_joint_b1': 0, 'torso_joint_b2': 0, 
                      ...
                      'arm_right_joint_4_u': 0,'arm_right_joint_5_r': 0,'arm_right_joint_6_b': 0,'arm_right_joint_7_t': 0,'head_hinge':0}
...
def motion_planner_main():
    ...
    # Subscribers
    rospy.Subscriber("/decision_making_state", DecisionMakingStateMessage, decision_making_state_handler, queue_size=1)
    # rospy.Subscriber("/robot/limb/left/endpoint_state", EndpointState, endpoint_state_state_handler, queue_size=1)
    if g_robot == BAXTER: # Enable robot
        rospy.Subscriber("/robot/joint_states", JointState, baxter_joints_state_handler, queue_size=1)
    else:
        rospy.Subscriber("/joint_states", JointState, motoman_joints_state_handler, queue_size=1)
    # rospy.Subscriber("/recognized_object_array", RecognizedObjectArray, detected_object_handler, queue_size=1)
...
#   srv = Server(prx_mappingConfig, reconfigure_callback)
    ...
    init_planning_library(g_move_group_name, g_robot)
    ...
    old_move_group = copy.deepcopy(g_move_group_name)

    print "=== Move arms to origin position: old move group: "+old_move_group
    set_arm_and_move_group(1) # right

    plan_and_move_to_joints_configuration(g_bin_positions[g_robot][1]['A']['origin_robot_configuration'], ROBOT_SPEED_FAST)
    set_arm_and_move_group(0) # left

    plan_and_move_to_joints_configuration(g_bin_positions[g_robot][0]['A']['origin_robot_configuration'], ROBOT_SPEED_FAST)
    print "=== Finished moving arms to origin position"

    if "left" in old_move_group:
        set_arm_and_move_group(0) # left
    else:
        set_arm_and_move_group(1) # right

    if False:
        # These are just to have a timely call to the motion planner main function 
        rospy.Subscriber("/camera/rgb/image_color", Image, camera_image_color_handler, queue_size=1)
        rospy.Subscriber("/camera2/rgb/image_color", Image, camera2_image_color_handler, queue_size=1)
    else:
        rospy.Timer(rospy.Duration(0.5), motion_planner_timer)
