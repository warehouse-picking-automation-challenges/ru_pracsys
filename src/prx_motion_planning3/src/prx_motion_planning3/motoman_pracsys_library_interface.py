...
def get_limb_postion_via_inverse_kinematics_camera(endpoint_desired_state, ik_seed):
    # global g_origin_robot_position # TODO: This should be the current arm configuration...
...
def plan_and_move_camera_to(arm_state,end_effector_pose,speed,ik_seed):
    print "plan_and_move_camera_to() calling plan_and_move_to_joints_configuration(). end_effector_pose:"
    print end_effector_pose
    for tries in range(1, 10):
        limb_joints = get_limb_postion_via_inverse_kinematics_camera(end_effector_pose,ik_seed)
...
def set_move_group(move_group_name):
    global g_arm_choice
    print "Move group change: "+move_group_name
    if move_group_name=="arm_left":
        g_arm_choice = "left"
    elif move_group_name=="arm_right":
        g_arm_choice = "right"
    return
...
def init_planning_library(arm_name,blah):
    ...
    print "Init planning library change:" + arm_name
